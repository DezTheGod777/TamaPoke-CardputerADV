...
// Phase 3 makes idle life visibly richer without changing the save journal.
// These states are transient only; they reset naturally and are never saved.
...
static int8_t ultimateHomeSpriteScale(uint32_t now) {
  ...
  // A short foreground zoom makes an affectionate/curious Pokemon feel like
  // it deliberately walked toward the player. Scale 3 stays within the PMD
  // renderer's normal crisp integer scaling rules.
...
static void drawUltimateIdleMomentFx(uint32_t now) {
  ...
  switch (ultimateIdleMoment) {
    case UIM_SKYWATCH: {
      // Tiny sky sparkles lead the eye upward while the Pokemon poses/breathes.
    ...
    case UIM_STRETCH: {
      // Short motion marks make BREATH/POSE read as a deliberate stretch.
    ...
    case UIM_APPROACH: {
      // A subtle check-in sparkle; no permanent HUD clutter.
...
static bool ultimatePhase3SpeciesMoment(uint32_t now) {
  ...
  // Starter families get recognizable family-flavored beats.
  if (d >= 1 && d <= 3) {
    static const uint8_t a[] = {PMD_BREATH, PMD_POSE, PMD_NOD};
    ultimateSetIdleMoment(UIM_STRETCH, now, 1900);
    ultimateFlair(now, a, 3, 1200, 900);
    return true;
  }
  if (d >= 4 && d <= 6) {
    static const uint8_t a[] = {PMD_ATTACK, PMD_POSE, PMD_HOP};
    ultimateSetIdleMoment(UIM_SPECIES, now, 1700);
    ultimateFlair(now, a, 3, 900, 900);
    return true;
  }
  if (d >= 7 && d <= 9) {
    static const uint8_t a[] = {PMD_HOP, PMD_NOD, PMD_BREATH};
    ultimateSetIdleMoment(UIM_SPECIES, now, 1700);
    ultimateFlair(now, a, 3, 950, 900);
    return true;
  }

  // Bird Pokemon look upward/pose as if tracking something overhead.
  if ((d >= 16 && d <= 18) || (d >= 21 && d <= 22) || d == 83) {
    static const uint8_t a[] = {PMD_POSE, PMD_HOP, PMD_NOD};
    ultimateSetIdleMoment(UIM_SKYWATCH, now, 1900);
    ultimateFlair(now, a, 3, 1050, 950);
    return true;
  }

  // Jigglypuff line settles into a quiet sit/nod beat.
  if (d == 39 || d == 40) {
    static const uint8_t a[] = {PMD_SIT, PMD_NOD, PMD_BREATH};
    ultimateSetIdleMoment(UIM_DOZE, now, 2000);
    ultimateFlair(now, a, 3, 1450, 900);
    return true;
  }

  // Psyduck line pauses and looks puzzled.
  if (d == 54 || d == 55) {
    static const uint8_t a[] = {PMD_NOD, PMD_SIT, PMD_POSE};
    ultimateSetIdleMoment(UIM_SKYWATCH, now, 1750);
    ultimateFlair(now, a, 3, 1100, 850);
    return true;
  }

  // Abra line prefers still, meditative moments.
  if (d >= 63 && d <= 65) {
    static const uint8_t a[] = {PMD_SIT, PMD_BREATH, PMD_POSE};
    ultimateSetIdleMoment(UIM_DOZE, now, 2200);
    ultimateFlair(now, a, 3, 1600, 1000);
    return true;
  }

  // Eevee and its original evolutions get friendly player check-ins.
  if (d >= 133 && d <= 136) {
    static const uint8_t a[] = {PMD_NOD, PMD_POSE, PMD_HOP};
    ultimateSetIdleMoment(UIM_APPROACH, now, 1700);
    petTargetX = 120;
    ultimateFlair(now, a, 3, 950, 800);
    return true;
  }

  // Ditto gets an understated shape-shift-like pose/breathe beat.
  if (d == 132) {
    static const uint8_t a[] = {PMD_POSE, PMD_BREATH, PMD_HOP};
    ultimateSetIdleMoment(UIM_SPECIES, now, 1800);
    ultimateFlair(now, a, 3, 1100, 900);
    return true;
  }

  // Legendary birds get a more dramatic signature pose.
  if (d >= 144 && d <= 146) {
    static const uint8_t a[] = {PMD_ATTACK, PMD_POSE, PMD_HOP};
    ultimateSetIdleMoment(UIM_SKYWATCH, now, 2100);
    ultimateFlair(now, a, 3, 1000, 1100);
    return true;
  }
...
static bool ultimateHabitatReaction(uint32_t now) {
  ...
  const uint8_t biome = DEX_TBL[pet.speciesId].biome;
  const int h = sceneHour();

  // These react to the habitat's visible conditions (snow, leaves, shore,
  // embers, night sky). They do not use live internet weather and add no audio.
  if (biome == 5) {
    static const uint8_t a[] = {PMD_HOP, PMD_POSE, PMD_BREATH};
    ultimateSetIdleMoment(UIM_SKYWATCH, now, 1850);
    ultimateFlair(now, a, 3, 1050, 900);
    return true;
  }
  if (biome == 2) {
  ...
  if (biome == 1) {
    ...
    ultimateSetIdleMoment(UIM_SPECIES, now, 1650);
    ultimateFlair(now, a, 3, 950, 850);
    return true;
  }
  if (biome == 3) {
    static const uint8_t a[] = {PMD_POSE, PMD_ATTACK, PMD_BREATH};
    ultimateSetIdleMoment(UIM_SPECIES, now, 1750);
    ultimateFlair(now, a, 3, 1000, 900);
    return true;
  }
  if ((h >= 19 || h < 6) && (biome == 0 || biome == 4)) {
    static const uint8_t a[] = {PMD_POSE, PMD_BREATH, PMD_NOD};
    ultimateSetIdleMoment(UIM_SKYWATCH, now, 1950);
    ultimateFlair(now, a, 3, 1250, 900);
    return true;
  }
  return false;
}
...
# Keep transient moment state clean on every ambient update.
text = must_replace(
    text,
...
# Low-energy Pokemon visibly doze instead of only selecting a sleep-ish action.
text = must_replace(
    text,
...
# Lonely and affectionate check-ins can come visually closer to the player.
text = must_replace(
    text,
...
# Night dozes receive their own visible moment state.
text = must_replace(
    text,
